...
** Query for Data
  ...
  [04:37] Once the data's returned from our API, we'll call ~this.setState~,
  updating the ~todos~ array with the data returned from the API. In the render
  method, we'll map over the ~todos~ array, showing the todo name and the todo
  description.
...
* Create & Interact with a Serverless REST API with AWS Lambda from React
  ...
  Amplify to interact with the Lambda function from a React app. Lambda
  functions allow us to easily create one-off functionality or interact with
  microservices. We’ll use the Lambda function to translate text from English
  to Spanish.
...
** Add a REST API Using a Lambda Function
   ...
   this peoplefunction. For the name of the Lambda function, we'll also call it
   peoplefunction. For the function template that we'd like to use, we'll use a
...
#+MACRO:VERSION Version 0.0.3 <2019-02-07 Thu 19:43>
